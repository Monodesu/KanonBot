    ...
    public static class GeneralUpdate
    {
        ...
        public static async Task<(long, TimeSpan)> UpdateUsers()
        {
            ...
            try
            {
                try
                {
                    var files = Directory.GetFiles($@".\work\avatar\");
            ...
            try
            {
                try
                {
                    var files = Directory.GetFiles($@".\work\legacy\v1_cover\osu!web\");
                    foreach (var file in files)
                        try
                        {
                            File.Delete(file);
        ...
        {
            var modes = new API.OSU.Enums.Mode[]
            {
                API.OSU.Enums.Mode.OSU,
                API.OSU.Enums.Mode.Taiko,
                API.OSU.Enums.Mode.Fruits,
                API.OSU.Enums.Mode.Mania
            };
            foreach (var mode in modes)
            {
                Log.Information($"正在更新用户数据....[{userID}/{mode}]");
                var userInfo = await API.OSU.V2.GetUser(userID, mode);
                OsuArchivedRec rec =
                    new()
                    {
                        uid = (int)userInfo!.Id,
