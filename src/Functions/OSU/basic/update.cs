...
namespace KanonBot.OSU
{
    public static partial class Basic
    {
        ...
        public async static Task update(CommandContext args, Target target)
        {
            var osu_username = "";
            ...
            try { File.Delete($"./work/avatar/{OnlineOSUUserInfo!.Id}.png"); } catch (Exception ex) { Log.Warning(ex.Message); }
            try { File.Delete($"./work/legacy/v1_cover/osu!web/{OnlineOSUUserInfo!.Id}.png"); } catch (Exception ex) { Log.Warning(ex.Message); }
            await target.reply("主要数据已更新，pp+数据正在后台更新，请稍后使用info功能查看结果。");

            try { await Database.Client.UpdateOsuPPlusData((await API.OSU.V2.TryGetUserPlusData(OnlineOSUUserInfo!))!.User!, OnlineOSUUserInfo!.Id); }
            catch { }//更新pp+失败，不返回信息
        }
    }
}
