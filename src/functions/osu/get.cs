...
namespace KanonBot.functions.osubot
{
    public class Get
    {
        ...
        async private static Task Bonuspp(Target target, string cmd)
        {
            ...
            {
                // 查询用户是否绑定
                var tempOsuInfo = await OSU.GetUser(
                ...
                case "ost":
                    try
                    {
                        ...
                        if (elo != 0)
                        {
                            ...
                            var body = (await OSU.GetMatchInfo(matchId.Value))![
                                "result"
                            ]!.ToObject<JObject>();
                            TimeSpan ts = new();
        ...
        async private static Task BPList(Target target, string cmd)
        {
            ...
            tempOsuInfo.PlayMode = mode!.Value;
            #endregion
            ...
            {
                if (cmd == "")
                    await target.reply($"你在 {tempOsuInfo.PlayMode.ToStr()} 模式上还没有bp呢。。");
                else
                    await target.reply(
                        $"{tempOsuInfo.Username} 在 {tempOsuInfo.PlayMode.ToStr()} 模式上还没有bp呢。。"
                    );
            }
            else
            {
