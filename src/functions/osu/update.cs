...
namespace KanonBot.functions.osubot
{
    public class Update
    {
        public static void Execute(Target target, string cmd)
        {
            ...
            target.reply(new Chain().msg("少女祈祷中..."));
            try { File.Delete($"./work/v1_cover/{OnlineOsuInfo.userId}.png"); } catch { }
            try { File.Delete($"./work/avatar/{OnlineOsuInfo.userId}.png"); } catch { }
            target.reply(new Chain().msg("主要数据已更新完毕，pp+数据正在后台更新，请稍后使用info功能查看结果。"));

            try { Database.Client.UpdateOsuPPlusData(API.Osu.GetUserPlusData(OnlineOsuInfo.userId), OnlineOsuInfo.userId); }
            catch { }//更新pp+失败，不返回信息
        }
    }
}
