    ...
    static public partial class V2
    {
        public static async Task<bool> GetTokenAsync()
        {
            ...
            try
            {
                ...
                TokenExpireTime = DateTimeOffset.Now.ToUnixTimeSeconds() + (long.TryParse(responseBody["expires_in"]?.ToString(), out var expiresIn) ? expiresIn : 0);

                return true;
            }
            catch (Exception ex) // 指定具体的异常类型
            {
                Log.Error($"获取token失败: {ex.Message}, 返回Body: \n({responseBody?.ToString() ?? "无"})");
                return false;
            }
        }
        ...
        public static async Task CheckTokenAsync()
        {
            ...
            {
                string tokenStatus = TokenExpireTime == 0 ? "正在获取OSUApiV2_Token" : "OSUApiV2_Token已过期, 正在重新获取";
                Log.Information(tokenStatus);

                if (await GetTokenAsync())
                {
                    // 避免在日志中显示完整的令牌信息
                    Log.Information($"获取成功, Token: {Token.Substring(0, Math.Min(Token.Length, 3))}...");
