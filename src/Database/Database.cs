...
public class Client
{
    ...
    public static async Task<bool> SetVerifyMail(string mailAddr, string verify)
    {
        ...
        var count = await db.InsertAsync(newverify);
        return count > 0;
    }
    ...
    {
        using var db = GetInstance();
        var li = db.Users.Where(it => it.email == mailAddr).Select(it => it.uid);
    ...
    {
        using var db = GetInstance();
        return await db.Users.Where(it => it.uid == uid).FirstOrDefaultAsync();
    }
        ...
        switch (platform)
        {
            case Platform.OneBot:
                if (long.TryParse(UID, out var qid))
                    return await db.Users.Where(it => it.qq_id == qid).FirstOrDefaultAsync();
                else
                    return null;
            case Platform.Guild:
                return await db.Users.Where(it => it.qq_guild_uid == UID).FirstOrDefaultAsync();
            case Platform.KOOK:
                return await db.Users.Where(it => it.kook_uid == UID).FirstOrDefaultAsync();
            case Platform.Discord:
                return await db.Users.Where(it => it.discord_uid == UID).FirstOrDefaultAsync();
            default:
                return null;
        }
    }
    ...
    public static async Task<Models.User?> GetUserByOsuUID(long osu_uid)
    {
        ...
        return await db.Users.Where(it => it.uid == user.uid).FirstOrDefaultAsync();
    }
    ...
    {
        using var db = GetInstance();
        var data = await db.Users.FirstOrDefaultAsync(it => it.uid == long.Parse(userid));
    ...
    public static async Task<Badge?> GetBadgeInfo(string badgeid)
    {
        using var db = GetInstance();
        return await db.Badges.Where(it => it.id == int.Parse(badgeid)).FirstOrDefaultAsync();
    }

    public static async Task<bool> SetOwnedBadge(string email, string? owned_ids)
    {
        using var db = GetInstance();
        var data = await db.Users.FirstOrDefaultAsync(it => it.email == email);
    ...
    {
        using var db = GetInstance();
        var data = await db.Users.FirstOrDefaultAsync(it => it.uid == uid);
        data!.owned_badge_ids = owned_ids;
        var res = await db.UpdateAsync(data);
        return res > -1;
    }
    ...
    public static async Task<bool> SetOwnedBadgeByOsuUid(string osu_uid, string? owned_ids)
    {
        ...
        using var db = GetInstance();
        var userinfo = await db.Users.Where(it => it.uid == user.uid).FirstOrDefaultAsync();
    ...
    public static async Task<SeasonalPass?> GetSeasonalPassInfo(long oid, string mode)
    {
        using var db = GetInstance();
        return await db.OSUSeasonalPass
    ...
    public static async Task<bool> SeasonalPass_Query_Score_Status(string mode, long score_id)
    {
        ...
        //insert
        var d = new SeasonalPassScoreRecords() { score_id = score_id, mode = mode };
        try
        {
            await db.InsertAsync(d);
            return true;
    ...
    {
        using var db = GetInstance();
        return await db.Users.Where(it => it.owned_badge_ids != null).ToListAsync();
    }

    public static async Task<List<Badge>> GetAllBadges()
    {
        using var db = GetInstance();
