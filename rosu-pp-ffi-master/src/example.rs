...
use interoptopus::{
    ffi_service, ffi_service_ctor, ffi_service_ignore, ffi_service_method, ffi_type, pattern,
};
use std::ffi::CString;
...
impl SimpleService {
    ...
    pub fn new() -> Result<Self, Error> {
        Ok(Self {
            some_value: 0,
            c_string: CString::new("Hello from rust").unwrap(),
            data: vec![1, 2, 3],
        })
    }
    ...
    pub fn new_with_string(ascii: AsciiPointer) -> Result<Self, Error> {
        let cstr = ascii.as_c_str().unwrap().to_owned();
        Ok(Self {
            some_value: 0,
            c_string: cstr,
            data: vec![1, 2, 3],
    ...
    pub fn hello(&mut self, ascii: AsciiPointer) {
        let str = ascii.as_c_str().unwrap().to_str().unwrap();
        println!("{str}");
    }
...
pub fn my_inventory() -> Inventory {
    InventoryBuilder::new()
        ...
        .register(pattern!(SimpleServiceLifetime))
        .inventory()
}
